 ...
 * Classe: Zobrist
 *
 * Le hashage de zobrist permet de réduire le plateau actuel en une clé de 64 bits
 * De ce fait, nous sauvegradons le plateau sous un format très réduit permettant
 * ultèrieurement de faire des recherches plus rapides.
 *
 * Bien qu' en JAVA, un long de 64 bits soit signés, cela ne semble pas poser de problèmes
 * pour obtenir des indexs grâce à cette clé.
 *
 *Le fonctionnement de zobrist est le suivant:
 *      Pour chaque position, chaque pièce, chaque couleur -> on va attribuer un nombre de 64 bits aléatoire
 *      Même chose pour le statut de jeu (droits de roque, E.P, tour,...)
 *
 *      De ce fait, il nous suffit de faire l'opération XOR (^) afin d'obtenir la clé finale.
 */
public class Zobrist {
    ...
    /**
     * Permet d'avoir de manière incrémentale la clé à la suite d'un move.
     *
     * @param from_loc -> position de départ
     * @param to_loc -> position d'arrivée
     * @param piece -> pièce qui bouge
     * @return une clé intérmédiaire afin de pouvoir l'ajouter à la clé actuelle
     */
    ...
    /**
     * Permet d'avoir la clé actuelle pour le plateau
     *
     * @param b -> Le plateau de jeu actuel
     * @return La clé résultante de notre plateau
     */
    public static long getKeyForBoard(Engine b) {
        long key = 0;
        ...
        if (b.hasCastlingRight(1))
            key ^= whiteKingSideCastling;
        if (b.hasCastlingRight(0))
            key ^= whiteQueenSideCastling;
        if (b.hasCastlingRight(3))
            key ^= blackKingSideCastling;
        if (b.hasCastlingRight(2))
            key ^= blackQueenSideCastling;

        if (b.enPassantSquare != -1)
            key ^= passantColumn[b.getEnPassantColumn(b.enPassantSquare)];
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Clé du Board: " + getKeyForBoard(engine));
        System.out.println("Clé de l'engine: " + engine.zobristKey);
...
//        System.out.println(getKeyForBoard(engine));
//        engine.makeMove("b1c3");
//
...
//        System.out.println(getKeyForBoard(engine));
//
//        Engine engine2 = new Engine("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
//        engine2.makeMove("d2d4");
//        engine2.makeMove("g8f6");
//        engine2.makeMove("c2c4");
//        engine2.makeMove("g7g6");
//        System.out.println(getKeyForBoard(engine2));
//
//        Engine engine3 = new Engine("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
//engine.printBoard();
//        System.out.println(getKeyForBoard(engine3));
//        System.out.println("key " +engine3.zobristKey);
//        engine3.makeMove("c2c4");
//        System.out.println(getKeyForBoard(engine3));
//        System.out.println("key " +engine3.zobristKey);
//
//        engine3.makeMove("g7g6");
//        System.out.println(getKeyForBoard(engine3));
//        System.out.println("key " +engine3.zobristKey);
//        engine3.makeMove("d2d4");
//        engine3.makeMove("g8f6");
//
//        System.out.println(getKeyForBoard(engine3));
//        System.out.println("key " +engine3.zobristKey);
